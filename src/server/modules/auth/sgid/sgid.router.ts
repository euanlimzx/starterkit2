...
export const sgidRouter = router({
  ...
  callback: publicProcedure
    ...
    .query(async ({ ctx, input: { state, code } }) => {
      ...
      const sgidEmailResult = normaliseEmail.safeParse(
        sgidUserInfo.data['myinfo.email']
      )
      if (!sgidEmailResult.success) {
        ctx.logger.warn(
          { error: sgidEmailResult.error },
          'Unable to process email from sgID'
        )
        throw new TRPCError({
          code: 'BAD_REQUEST',
          message: 'Email was unable to be processed. Please try again.',
        })
      }
      const user = await upsertSgidAccountAndUser({
        prisma: ctx.prisma,
        name: sgidUserInfo.data['myinfo.name'],
        email: sgidEmailResult.data,
